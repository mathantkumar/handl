...
export function AIProgressModal({ isOpen, progress, status, error }: AIProgressModalProps) {
    ...
    if (status === "error") {
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                <div className="relative w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl p-8 text-center border border-zinc-200 dark:border-zinc-800">
                    <div className="w-16 h-16 bg-red-100 dark:bg-red-900/30 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4">
                        <AlertTriangle className="w-8 h-8" />
                    </div>
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Initialization Failed</h2>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">{error || "Browser compatibility issue."}</p>
                    <button
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            {/* Modal Container: Single white/dark background, rounded-2xl, overflow-hidden */}
